...
public class Find_Selector_Conditions : HttpTests
{
    #region (In)Equality
    ...
    {
        var json = Rebels.Where(r => r.FieldExists("age")).ToString();
    ...
    {
        var json = Rebels.Where(r => r.Age.IsCouchType(CouchType.CNumber)).ToString();
    ...
    {
        var json = Rebels.Where(r => r.Age.In(new[] { 20, 30 })).ToString();
    ...
    {
        var json = Rebels.Where(r => r.Guid.In(new[]
            {
    ...
    {
        var json = Rebels.Where(r => r.Age.In(new[] { 20 })).ToString();
    ...
    {
        var json = Rebels.Where(r => r.Species.In(new[] { Species.Human, Species.Droid })).ToString();
    ...
    {
        var json = Rebels.Where(r => r.Skills.Any(s => s.IsMatch($"(?i)sab") || s.IsMatch($"(?)orce"))).ToString();
        Assert.Equal(
