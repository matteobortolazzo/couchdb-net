...
public class Find_Sort : HttpTest
{
    [Fact]
    public void SortAsc()
    {
    ...
    {
        var exception =
            Record.Exception(() => { _rebels.OrderBy(c => c.Name).ThenByDescending(c => c.Age).ToString(); });
    ...
    {
        var exception =
            Record.Exception(() => { _rebels.OrderByDescending(c => c.Name).ThenBy(c => c.Age).ToString(); });
        Assert.NotNull(exception);
        Assert.IsType<InvalidOperationException>(exception);
    }
}