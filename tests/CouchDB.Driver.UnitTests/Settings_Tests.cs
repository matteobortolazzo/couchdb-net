...
public class Settings_Tests
{
    ...
    public async Task Creation_BasicAuthentication()
    {
        ...
        await using var client =
            new CouchClient("http://localhost", s => s.UseBasicAuthentication("root", "relax"));
        Assert.Equal("http://localhost/", client.Endpoint.AbsoluteUri);
    }
    ...
    public async Task Creation_CookieAuthentication()
    {
        ...
        await using var client =
            new CouchClient("http://localhost", s => s.UseCookieAuthentication("root", "relax"));
        Assert.Equal("http://localhost/", client.Endpoint.AbsoluteUri);
    }
    ...
    [Fact]
    public async Task PropertyName_JsonPropertyName()
    {
        using var httpTest = new HttpTest();
        // Logout
        httpTest.RespondWithJson(new { ok = true });
    ...
    public async Task PropertyNullValueHandling_NotSet()
    {
        ...
        Assert.NotNull(call);
        Assert.Equal(
            @"{""age"":0,""isJedi"":false,""species"":0,""guid"":""00000000-0000-0000-0000-000000000000"",""_attachments"":{}}",
            call.RequestBody);
    }
    ...
    public async Task PropertyNullValueHandling_Includes()
    {
        ...
        await using var client = new CouchClient("http://localhost",
            x => x.SetJsonIgnoreCondition(JsonIgnoreCondition.WhenWritingNull));
        var rebels = client.GetDatabase<Rebel>();
        await rebels.AddAsync(new Rebel());
        ...
        Assert.NotNull(call);
        Assert.Equal(
            @"{""name"":null,""surname"":null,""age"":0,""isJedi"":false,""species"":0,""guid"":""00000000-0000-0000-0000-000000000000"",""skills"":null,""battles"":null,""vehicle"":null,""_attachments"":{}}",
            call.RequestBody);
    }
