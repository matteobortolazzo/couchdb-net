...
internal static class StreamExtensions
{
    ...
    private class LinesDecoder(Memory<byte> streamMemory) : IDisposable
    {
        private readonly Decoder _uniDecoder = Encoding.UTF8.GetDecoder();
        private string _remainder = string.Empty;
        private readonly IMemoryOwner<char> _decodingMemory = MemoryPool<char>.Shared.Rent(streamMemory.Length);

        public IEnumerable<string> ReadLines(int readCharCount)
        {
            Span<byte> readableStreamSpan = streamMemory.Span[..readCharCount];
