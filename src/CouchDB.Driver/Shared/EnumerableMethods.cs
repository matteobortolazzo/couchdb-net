...
internal static class EnumerableMethods
{
    ...
    {
        if (QueryableToEnumerableMethods.TryGetValue(methodInfo, out MethodInfo? equivalent))
        {
            return equivalent;
        }
    ...
    static EnumerableMethods()
    {
        ...
        MethodInfo anyWithoutPredicate =
            enumerableMethods.Single(mi => mi.Name == nameof(Enumerable.Any) && mi.GetParameters().Length == 1);

        MethodInfo firstWithoutPredicate =
            enumerableMethods.Single(mi => mi.Name == nameof(Enumerable.First) && mi.GetParameters().Length == 1);
        MethodInfo firstOrDefaultWithoutPredicate = enumerableMethods.Single(mi =>
            mi.Name == nameof(Enumerable.FirstOrDefault) && mi.GetParameters().Length == 1);

        MethodInfo singleWithoutPredicate =
            enumerableMethods.Single(mi => mi.Name == nameof(Enumerable.Single) && mi.GetParameters().Length == 1);
        MethodInfo singleOrDefaultWithoutPredicate = enumerableMethods.Single(mi =>
            mi.Name == nameof(Enumerable.SingleOrDefault) && mi.GetParameters().Length == 1);
        MethodInfo lastWithoutPredicate =
            enumerableMethods.Single(mi => mi.Name == nameof(Enumerable.Last) && mi.GetParameters().Length == 1);
        MethodInfo lastOrDefaultWithoutPredicate = enumerableMethods.Single(mi =>
            mi.Name == nameof(Enumerable.LastOrDefault) && mi.GetParameters().Length == 1);
